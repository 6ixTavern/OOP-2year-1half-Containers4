...
namespace classTest
{
    //public class Cheat : IComparable<Student>
    ...
    //    public Student() { }
    //    public Student(int a, string f, int k) { age = a; fio = f; kurs = k; }
    //    public override string ToString()
    //    {
    //        return String.Format($"{fio} {age} {kurs}");
    //    }
    ...
    //    {
    //        kurs++;
    //        if (kurs > 6) throw new GradeUniversityException("only 6 courses" + fio, 777);
    //    }
    ...
    //    }
    //    public static NameComparer GetNameComparer()
    //    {
    ...
    public abstract class Cheat : IComparable<Cheat>
    {
        #region Field
        ...
        {
            if (CheatVersion == other.CheatVersion)
    ...
    public class NameComparer : IComparer<Cheat>
    {
        public int Compare(Cheat x, Cheat y)
        {
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //1
            Array.Sort(multihack);
            for (int i = 0; i < n; i++)
                    Console.WriteLine(multihack[i]);
            Console.WriteLine();

            //2
            Array.Sort(multihack, new NameComparer());
            Array.Sort(multihack, Cheat.GetNameComparer());
            for (int i = 0; i < n; i++)
                Console.WriteLine(multihack[i]);
            Console.WriteLine();

            //3
            Stack<Cheat> st = new Stack<Cheat>();
            st.Push(multihack[0]);
            st.Push(multihack[1]);
            Console.WriteLine("Количество элементов: " + st.Count);
            while (st.Count != 0)
            ...
            //4-5
            Console.WriteLine();
            MyСontainer<Cheat> list = new MyСontainer<Cheat>();
            list.AddFirst(multihack[1]);
            list.AddLast(multihack[2]);
            list.AddFirst(multihack[3]);
            Console.WriteLine(list.Contains(new Aimbot("rampage hack", "stepan", 1, true, 1.0)));
            Console.WriteLine(list.Contains(multihack[1]));
            list.Remove(list.First());
            foreach (Cheat c in list)
            {
